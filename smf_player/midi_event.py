...
def _format_display(index: int, abs_time: float, event: PlaybackEvent) -> str:
    ...
    return (
        f'{index:04d}  {abs_time:8.3f}s  {ch_str}  '
        f'{type_str:<9} {detail}'
    )
...
def load_midi_events(filepath: str) -> list[PlaybackEvent]:
    ...
    all_events: list[tuple[float, int, mido.Message]] = []
    ...
    for track in mid.tracks:
        ...
        for tick, msg in tick_events:
            ...
            all_events.append((abs_sec, id(msg), msg))
    ...
    for i, (abs_sec, _, msg) in enumerate(all_events):
        ...
        ev.display_text = _format_display(i, abs_sec, ev)
        result.append(ev)
